...
class RoleManager {
  ...
  heal(medicRoleId, targetRoleId) {
    ...
    if (target.health >= 50) return { ok: false, error: 'Target must be below 50% health to heal.' };
    ...
    if (!target.healCount) target.healCount = 0;
    if (target.healCount >= 2) return { ok: false, error: 'This player has already been healed twice.' };

    target.health = Math.min(100, target.health + 50);
    target.healCount++;
    this.io.emit('updatePlayers', players);
    this.io.emit('healed', { medic: medic.name, target: target.name, amount: 50, healsLeft: 2 - target.healCount });
