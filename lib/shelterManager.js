...
class ShelterManager {
  ...
  reset() {
    ...
    this.constructionTimer = null;
    if (this._autoRepairTimer) clearInterval(this._autoRepairTimer);
    this._autoRepairTimer = null;
  ...
  damageFurniture(furnitureId) {
    ...
    if (f.currentDurability <= 0) {
      ...
      for (const rid in players) {
        if (players[rid].hidingIn === furnitureId) {
          players[rid].hidingIn = null;
        }
      }
  ...
  startAutoRepair() {
    if (this._autoRepairTimer) return;
    this._autoRepairTimer = setInterval(() => {
      const players = this.getPlayers();
      let hasConstructor = false;
      for (const rid in players) {
        const p = players[rid];
        if (p.isAlive && p.roleName === 'Constructor') { hasConstructor = true; break; }
      }
      if (hasConstructor && this.shelterIntegrity < 100) {
        this.repairShelter(20);
      }
    }, 20000);
  }

  stopAutoRepair() {
    if (this._autoRepairTimer) {
      clearInterval(this._autoRepairTimer);
      this._autoRepairTimer = null;
    }
  }
  ...
  housekeeperRestore(housekeeperRoleId) {
    ...
    if (allClicked) {
      ...
      for (const fid in this.furniture) {
        ...
        f.hiddenBy = null;
        if (f.destroyed) {
          const cfg = this.furnitureConfig.find(c => c.id === fid);
          f.destroyed = false;
          f.currentDurability = cfg ? cfg.durability : f.durability;
        }
        restored++;
