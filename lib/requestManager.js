...
class RequestManager {
  constructor({ io, getPlayers, itemsConfig }) {
    this.io = io;
  ...
  _findTarget(type, fromRoleId) {
    ...
    if (type === 'heal') {
      ...
      for (const rid in players) {
        const p = players[rid];
        if (rid !== fromRoleId && p.isAlive && p.connected && p.roleName === 'Scavenger') {
          const hasFood = p.inventory && p.inventory.some(i => i.id === 'food' || i.id === 'water');
          if (hasFood) return rid;
        }
      }
    } else if (type === 'scavenger_food') {
      for (const rid in players) {
        const p = players[rid];
        if (rid !== fromRoleId && p.isAlive && p.connected && p.roleName === 'Scavenger') {
          const hasFood = p.inventory && p.inventory.some(i => i.id === 'food' || i.id === 'water');
  ...
  _getRequestMessage(type, fromName) {
    const messages = {
      ...
      scavenger_food: `${fromName} (Scavenger) needs food from a fellow Scavenger!`,
  ...
  _fulfillRequest(request, players) {
    ...
    if (request.type === 'heal' && target.roleName === 'Medic') {
      ...
      if (from.socketId) this.io.to(from.socketId).emit('actionSuccess', { message: 'You have been healed by ' + target.name + '!' });
    } else if (request.type === 'scavenger_food' && target.roleName === 'Scavenger') {
      const itemId = request.sharedItemId || 'food';
      const itemDef = this.getItemDef ? this.getItemDef(itemId) : null;
      if (itemDef) {
        if (itemDef.hungerRestore) from.hunger = Math.min(100, from.hunger + itemDef.hungerRestore);
        if (itemDef.healthRestore) from.health = Math.min(100, from.health + itemDef.healthRestore);
        if (from.socketId) this.io.to(from.socketId).emit('actionSuccess', { message: target.name + ' shared ' + (itemDef.icon || '') + ' ' + itemDef.name + ' with you!' });
        if (target.socketId) this.io.to(target.socketId).emit('actionSuccess', { message: 'You shared ' + (itemDef.icon || '') + ' ' + itemDef.name + ' with ' + from.name + '!' });
      } else {
        from.hunger = Math.min(100, from.hunger + 25);
        if (from.socketId) this.io.to(from.socketId).emit('actionSuccess', { message: target.name + ' shared food with you!' });
      }
