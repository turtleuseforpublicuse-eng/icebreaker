...
class QuizManager {
  ...
  _endQuiz() {
    ...
    // Offer winners the choice: keep points or take 10 food + 10 health
    for (const w of winners) {
      const p = players[w.roleId];
      if (p && p.isAlive) {
        this.io.emit('winnerChoice', { roleId: w.roleId, source: 'quiz', points: w.points });
      }
    }
  ...
  _endVoting() {
    ...
    if (sorted.length > 0 && players[sorted[0][0]]) {
      ...
      // Offer winners choice: keep points or take food+health
      const essayWinners = [[winner, 30], [runnerUp, 20], [third, 10]].filter(([rid]) => rid && players[rid] && players[rid].isAlive);
      for (const [rid, pts] of essayWinners) {
        this.io.emit('winnerChoice', { roleId: rid, source: 'essay', points: pts });
      }
