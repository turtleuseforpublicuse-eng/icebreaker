...
const disasterManager = new DisasterManager({
  ...
  onDisasterEnd: (ev, scores) => {
    ...
    currentRound++;
    if (scores) {
      for (const rid in scores) {
        if (players[rid] && players[rid].isAlive) {
          players[rid].totalPoints = (players[rid].totalPoints || 0) + scores[rid];
        }
      }
      const scoreData = {};
      for (const rid in players) {
        scoreData[rid] = { totalPoints: players[rid].totalPoints || 0, quizScore: players[rid].quizScore || 0 };
      }
      io.emit('totalScores', scoreData);
    }
...
const quizManager = new QuizManager({
  ...
  getPlayers: () => players,
  getGameStarted: () => gameStarted,
  scoringConfig: config.gameSettings
});
...
function _checkWinner() {
  ...
  if (alive.length <= 1 || currentRound >= totalRounds) {
    ...
    for (const [rid, p] of alive) {
      const score = (p.totalPoints || 0) + (p.quizScore || 0);
      if (score > bestScore) { bestScore = score; winner = rid; }
    }
    if (!winner && alive.length === 0) {
      for (const [rid, p] of Object.entries(players)) {
        const score = (p.totalPoints || 0) + (p.quizScore || 0);
        if (!winner || score > bestScore) { bestScore = score; winner = rid; }
      }
    }
    if (winner && players[winner]) {
      const finalScores = {};
      for (const rid in players) {
        finalScores[rid] = { totalPoints: players[rid].totalPoints || 0, quizScore: players[rid].quizScore || 0, health: players[rid].health, hunger: players[rid].hunger };
      }
      io.emit('totalScores', finalScores);
      io.emit('gameOver', {
        winnerId: winner,
...
function stopCycleTimers() {
  ...
  shelterManager.stopAutoRepair();
  if (quizCooldownTimer) { clearTimeout(quizCooldownTimer); quizCooldownTimer = null; }
}

function startCycleTimers() {
  isDay = true;
  io.emit('skyState', { dark: false, isDay: true });
  dayNightTimer = setInterval(() => {
    isDay = !isDay;
    io.emit('skyState', { dark: !isDay, isDay });
  }, config.gameSettings?.dayNightCycleMs || 30000);
  shelterManager.startAutoRepair();
}
...
io.on('connection', (socket) => {
  ...
  socket.on('requestHostData', () => {
    socket.emit('hostData', { roomCode, gameStarted, currentRound, totalRounds, shelterIntegrity: shelterManager.shelterIntegrity, isDay });
